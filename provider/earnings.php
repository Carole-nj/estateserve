...
$paid_jobs = $pdo->prepare("SELECT COUNT(*)
    FROM payments p JOIN bookings b ON p.booking_id = b.id
    WHERE b.provider_id = ? AND p.status = 'success'");
$paid_jobs->execute([$user_id]);
$paid_jobs = $paid_jobs->fetchColumn();

$avg_job = $paid_jobs > 0 ? $total / $paid_jobs : 0;

// Payment history for this provider
$stmt = $pdo->prepare("
...
<head>
  ...
  <title>Earnings — EstateServe</title>
...
<div class="sidebar">
  ...
  <nav class="nav flex-column">
    ...
    <a href="earnings.php"   class="nav-link active"><i class="bi bi-cash-stack"></i> Earnings</a>
...
<div class="main">
  <div class="topbar">
    <div>
      <h5 class="mb-0 fw-bold">Earnings</h5>
  ...
  <div class="row g-3 mb-4">
    <div class="col-md-3">
      <div class="stat-card" style="background:linear-gradient(135deg,#00A550,#007a3d)">
        <div class="opacity-75 small mb-1">Total Earnings (KES)</div>
    ...
    <div class="col-md-3">
      <div class="stat-card" style="background:linear-gradient(135deg,#f59e0b,#d97706)">
        <div class="opacity-75 small mb-1">Paid Jobs</div>
        <div class="fs-2 fw-bold"><?= $paid_jobs ?></div>
      </div>
    </div>
    <div class="col-md-3">
      <div class="stat-card" style="background:linear-gradient(135deg,#8b5cf6,#6d28d9)">
        <div class="opacity-75 small mb-1">Avg / Job (KES)</div>
        <div class="fs-2 fw-bold"><?= number_format($avg_job, 0) ?></div>
      </div>
    </div>
  </div>
  ...
  <div class="card border-0 shadow-sm rounded-3">
    ...
    <div class="card-body p-0">
      <?php if (empty($payments)): ?>
        <p class="text-center text-muted py-4">No payments received yet.</p>
      <?php else: ?>
